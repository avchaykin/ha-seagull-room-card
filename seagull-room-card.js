    ...
    const renderButton = (item, index, opts = {}) => {
      ...
      if (badgeVisible && badgeCfg && typeof badgeCfg === "object") {
        ...
        if (hasBadgeIcon || hasBadgeText) {
          ...
          badgeHtml = `<span aria-hidden="true" style="position:absolute;top:2px;right:2px;width:${badgeSize}px;height:${badgeSize}px;border-radius:999px;background:${this._esc(badgeColor)};display:inline-flex;align-items:center;justify-content:center;pointer-events:none;z-index:3;">
              ${hasBadgeIcon
      ...
      : `<span style="color:${this._esc(badgeIconColor)};font-size:${Math.max(7, Math.round(badgeSize * 0.52))}px;line-height:1;font-weight:700;">${this._esc(String(badgeText))}</span>`}
          ...
          badgeHtml = `<span aria-hidden="true" style="position:absolute;top:0;right:0;transform:translate(50%,-50%);width:${badgeSize}px;height:${badgeSize}px;border-radius:999px;background:${this._esc(badgeColor)};pointer-events:none;z-index:3;"></span>`;
        }
      }
